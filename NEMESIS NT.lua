...
local GRADIENT_COLOR_B = Color3.fromRGB(70, 70, 70)
...
local THEMES = {
	...
	bw     = { Color3.fromRGB(255, 255, 255), Color3.fromRGB(70, 70, 70) },
...
local DEFAULT_BG_FILE = "defaulttagbg.png"
local DEFAULT_BG_URL = "https://i.ibb.co/PsnHkTjx/image.png"
...
local ROLE_PRESETS = {
	["ᴅᴇᴠᴇʟᴏᴘᴇʀ"] = {
		...
		totalFrames = 15,
		framesPerSec = 10,
		staticBgFile = "zenin_tag.png",
		staticBgURL = "https://cdn.discordapp.com/attachments/1493961870548144131/1539765742113660948/zenin_tag.png?ex=6a878218&is=6a863098&hm=47420884a4bec3246131da163ff1fff06caeacb7c6635e30d49ef055fb9ef072&",
		textAlignment = "Left",
	},
	["ANGEL"] = {
		gradientA = Color3.fromRGB(255, 255, 255),
	...
	["OWNER"] = {
		...
		tagHeight = 50,
		tagOffsetY = 1.9,
		logoAsset = "whitecrownkiko.png",
		logoURL = "https://i.ibb.co/0yRpn9gy/whitecrownkiko.png",
		useAnimatedBg = false,
		spriteFile = "",
		spriteURL = "",
		frameColumns = 5,
		...
		totalFrames = 15,
		framesPerSec = 4,
		staticBgFile = "kikonewtagdark.png",
		staticBgURL = "https://i.ibb.co/PsnHkTjx/image.png",
		textAlignment = "Left",
	},
	["ᴘᴀᴘᴀ ᴄʜʀᴏɴ"] = {
	...
	["eetxn"] = {
		...
		logoURL = "",
		useAnimatedBg = true,
		spriteFile = "xkc5ua.png",
		spriteURL = "https://files.catbox.moe/xkc5ua.png",
		frameColumns = 5,
		frameRows = 4,
		totalFrames = 20,
		framesPerSec = 8,
	...
	["NEMESIS BOOSTER"] = {
		...
		logoURL = "",
		useAnimatedBg = true,
		spriteFile = "boostertag.png",
		spriteURL = "https://cdn.discordapp.com/attachments/1493961870548144131/1539987446115078225/boostertag.png?ex=6a885092&is=6a86ff12&hm=51cdb748219cfb7efcd50928cb08db565e73e10757810705724bbc5b56732c48&",
		frameColumns = 5,
		frameRows = 3,
		totalFrames = 15,
		framesPerSec = 4,
	...
	["FENTEX"] = {
		...
		tagHeight = 50,
		tagOffsetY = 1.9,
		logoAsset = "fentexredcheck.png",
		logoURL = "https://files.catbox.moe/n2e98e.png",
		useAnimatedBg = true,
...
local function ensureImage(file, url)
	if not file or file == "" or not url or url == "" then return end
	if isfile(file) then return end
...
ensureImage(DEFAULT_BG_FILE, DEFAULT_BG_URL)

local function loadImage(file, fallback)
	if file and file ~= "" then
		if not isfile(file) then
			ensureImage(file, fallback)
		end
		if getasset and isfile(file) then
...
local function getThemeColors()
	return THEMES[CONFIG.Theme] or THEMES.bw
end
...
-- STATIC left → right gradient (no animation)
local function applyStaticGradient(label, colorA, colorB)
...
function buildTag(plr)
	...
	local isDefault = not customData
	...
	bg.BackgroundTransparency = isDefault and 1 or 0.12
	...
	-- ========== BACKGROUND ==========
	if isDefault then
		-- ONLY default tags get the custom image
		local bgImage = Instance.new("ImageLabel")
		bgImage.Name = "DefaultBg"
		bgImage.Parent = bg
		bgImage.Size = UDim2.new(1, 0, 1, 0)
		bgImage.BackgroundTransparency = 1
		bgImage.Image = loadImage(DEFAULT_BG_FILE, DEFAULT_BG_URL)
		bgImage.ScaleType = Enum.ScaleType.Crop
		bgImage.ZIndex = 1
		Instance.new("UICorner", bgImage).CornerRadius = TAG_CORNER
	else
		-- Role tags keep original behavior
		...
		if customData.useAnimatedBg and customData.spriteFile and customData.spriteURL then
			...
			task.spawn(function()
				local tries = 0
				while bb and bb.Parent and spriteImg.ContentImageSize.X == 0 and tries < 60 do
					task.wait(0.1)
					tries = tries + 1
				end
	...
	if isDefault then
		-- Smaller, cleaner logo size for default tags
		logoHolder.Size = UDim2.new(0.16, 0, 0.62, 0)
		logoHolder.Position = UDim2.new(0.05, 0, 0.19, 0)
	else
		-- Original size for role tags
	...
	logoImg.ScaleType = Enum.ScaleType.Fit   -- Fit looks cleaner for white logo
	logoImg.ZIndex = 5
	...
	if isDefault then
		-- Adjust text position because logo is smaller
		textSizeX = 0.72
		textPosX = 0.24
	elseif alignEnum == Enum.TextXAlignment.Center then
		textSizeX = 0.92
		textPosX = 0.04
	...
	dname.Text = "@" .. plr.Name
	dname.TextColor3 = isDefault and Color3.fromRGB(90, 90, 90) or Color3.fromRGB(220, 220, 220)
	...
	-- Gradients
	if isDefault then
		-- Clean white → dark grey static gradient
		applyStaticGradient(kzk, Color3.fromRGB(255, 255, 255), Color3.fromRGB(180, 180, 180))
		applyStaticGradient(dname, Color3.fromRGB(90, 90, 90), Color3.fromRGB(60, 60, 60))
	else
		applyStaticGradient(kzk, finalColors[1], finalColors[2] or finalColors[1])
		applyStaticGradient(dname, finalColors[1], finalColors[2] or finalColors[1])
	end
		...
		if cam and hd and hd.Parent then
			...
			if success and dist then
				...
				elseif dist < SHRINK_DISTANCE and isSquare then
					...
					if isDefault then
						tweenSvc:Create(logoHolder, TweenInfo.new(0.3), {
							Size = UDim2.new(0.16, 0, 0.62, 0),
							Position = UDim2.new(0.05, 0, 0.19, 0)
						}):Play()
					else
						tweenSvc:Create(logoHolder, TweenInfo.new(0.3), {
